...
assert(false); // ensure the test fails in case the previous method does not raise an exception
}catch
(err)
{
    assert(err);
...
it ('only the manage can pick the winner', async() =>
{
    try {
        await
...
it ('sends money to the winner and reset the player list', async () => {
    await lottery.methods.enter().send({
        from: accounts[0],
    ...
    const difference = finalBalance - initialBalance;
     ...
     assert(difference > web3.utils.toWei('1.9999', 'ether'));
